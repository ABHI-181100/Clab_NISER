...
import math
from mylibr import myLibrary, gauss_seidel
class datafit:
    def __init__(self):
        pass
    ...
    def poly_fit(self,x, y, deg):
        ...
        # Solve normal equations using numpy.linalg.solve (avoid custom gauss_jordan)
        A_mat = np.array(A, dtype=float)
        B_vec = np.array(B, dtype=float)
        coeff_vec = np.linalg.solve(A_mat, B_vec)

        # Convert to Python list and return (a0..adeg)
        coeff = [float(c) for c in coeff_vec]
